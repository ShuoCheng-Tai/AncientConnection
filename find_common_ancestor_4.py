...
def find_common_ancestor(main_ID_p1, main_ID_p2, tree):
    ancestors_p1 = find_ancestors(main_ID_p1, tree)
    ancestors_p2 = find_ancestors(main_ID_p2, tree)

    common_ancestor = None
    common_ancestor_path = []
    generations = 0

    while ancestors_p1 and ancestors_p2:
        ancestor_p1 = ancestors_p1.pop() # remove Unmatch ID (Not Common Ancestor)
        ancestor_p2 = ancestors_p2.pop() 
        if ancestor_p1 == ancestor_p2: # if shared common ancestor found
            common_ancestor = ancestor_p1
            common_ancestor_path.append(ancestor_p1)
        else:
            break
        generations += 1
...
def match_anc(DNA_main_col, DNA_ISOGG_col, DNA_type, ID_com_anc, df, Date_p1, Date_p2):
    ...
    if len(match_rows) == 1:
        ...
        for k in match_rows:
            ...
            if int(df.iloc[k,8]) == 0: # 0 = present = 1950
                date = 1950
            if date > yng_anc:
                yng_anc = date
                info_anc = df.iloc[k,:]
